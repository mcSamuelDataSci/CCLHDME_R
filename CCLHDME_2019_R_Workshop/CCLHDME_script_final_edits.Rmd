...
- <img src="images/script_open.png" width=800)>
...
- <img src="images/chhs.png" width=800)>
...
```{r campylobacteriosis_forplot, echo = TRUE}
...
```{r campylobacteriosis_plot, echo = TRUE}
...
```{r foodborne_plot_int, echo = TRUE}
...
#here I pull tract level data for only the two vars we specified above (B01001_001E and B09001_001E) for Alameda County, CA 
...
- Now we want a map of the % of the population that is under 18
- We need to prepare our data and create a new variable with the percentage that we want to map
```{r census_data, echo = TRUE}
# Select the columns of interest and put `totpop` and `under18` in their own columns
...
```{r census_data_process, echo = TRUE}
head(counties2010)
